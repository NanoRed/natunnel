...
type Logger interface {
	Debug(stackoffset int, format string, v ...interface{})
...
// Debug debug log
func (l *DefaultLogger) Debug(stackoffset int, format string, v ...interface{}) {
	if config.IsDebugMode {
		l.debug.Output(2+int(stackoffset), fmt.Sprintf(format, v...))
	}
}
